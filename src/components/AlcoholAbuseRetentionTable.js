...
import { calculateRetentionByAlcoholAbuse } from '@/lib/filterUtils';

export default function AlcoholAbuseRetentionTable({ processedData, filters }) {
  const [showYes, setShowYes] = useState(true);
  const [showNo, setShowNo] = useState(true);
  const { state } = useRetention();
  const { rawData } = state;

  const calculateAlcoholAbuseData = () => {
    ...
    const retentionData = calculateRetentionByAlcoholAbuse(processedData, rawData);
  ...
  const alcoholData = calculateAlcoholAbuseData();
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontWeight: 'bold', marginRight: '15px' }}>Alcohol Abuse:</span>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: '600px' }}>
        ...
        <tbody>
          {Object.entries(alcoholData).map(([year, data]) => (
            <tr key={year}>
