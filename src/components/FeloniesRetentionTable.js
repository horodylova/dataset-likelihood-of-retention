...
import { calculateRetentionByFelonies } from '@/lib/filterUtils';

export default function FeloniesRetentionTable({ processedData, filters }) {
  const [showYes, setShowYes] = useState(true);
  const [showNo, setShowNo] = useState(true);
  const { state } = useRetention();
  const { rawData } = state;

  const calculateFeloniesData = () => {
    ...
    const retentionData = calculateRetentionByFelonies(processedData, rawData);
  ...
  const feloniesData = calculateFeloniesData();
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontWeight: 'bold', marginRight: '15px' }}>Felonies:</span>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: '600px' }}>
        ...
        <tbody>
          {Object.entries(feloniesData).map(([year, data]) => (
            <tr key={year}>
