...
export default function HIVRetentionTable({ processedData, filters }) {
  const [showYes, setShowYes] = useState(true);
  const [showNo, setShowNo] = useState(true);
  const { state } = useRetention();
  const { rawData } = state;

  const calculateHIVData = () => {
    ...
    const hivColumnIndex = headers.findIndex(h => 
      h && h.toLowerCase().trim() === 'hiv / aids'
    );
    ...
    const yesData = processedData.filter(resident => {
      const cellValue = resident.rawData[hivColumnIndex];
  ...
  const hivData = calculateHIVData();
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontWeight: 'bold', marginRight: '15px' }}>HIV / AIDS:</span>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: '600px' }}>
        ...
        <tbody>
          {Object.entries(hivData).map(([year, data]) => (
            <tr key={year}>
