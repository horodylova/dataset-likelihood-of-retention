...
import { calculateRetentionByHearing } from '@/lib/filterUtils';

export default function HearingRetentionTable({ processedData, filters }) {
  const [showYes, setShowYes] = useState(true);
  const [showNo, setShowNo] = useState(true);
  const { state } = useRetention();
  const { rawData } = state;

  const calculateHearingData = () => {
    ...
    const retentionData = calculateRetentionByHearing(processedData, rawData);
  ...
  const hearingData = calculateHearingData();
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontWeight: 'bold', marginRight: '15px' }}>Hearing:</span>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: '600px' }}>
        ...
        <tbody>
          {Object.entries(hearingData).map(([year, data]) => (
            <tr key={year}>
