...
export default function PhysicalMobilityRetentionTable({ processedData, filters }) {
  const [showYes, setShowYes] = useState(true);
  const [showNo, setShowNo] = useState(true);
  const { state } = useRetention();
  const { rawData } = state;

  const calculatePhysicalMobilityData = () => {
    ...
    const pmobColumnIndex = headers.findIndex(h => 
      h && h.toLowerCase().trim() === 'physical / mobility'
    );

    if (pmobColumnIndex === -1) {
      const emptyData = {};
      for (let year = 1; year <= 10; year++) {
        const yearKey = `Year ${year}`;
        emptyData[yearKey] = {
    ...
    const yesData = processedData.filter(resident => {
      const cellValue = resident.rawData[pmobColumnIndex];
  ...
  const pmobData = calculatePhysicalMobilityData();
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontWeight: 'bold', marginRight: '15px' }}>Physical / Mobility:</span>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: '600px' }}>
        ...
        <tbody>
          {Object.entries(pmobData).map(([year, data]) => (
            <tr key={year}>
