...
export default function SubstanceAbuseRetentionTable({ processedData, filters }) {
  ...
  const calculateSubstanceAbuseData = () => {
  ...
  const substanceData = calculateSubstanceAbuseData();
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontWeight: 'bold', marginRight: '15px' }}>Substance Abuse:</span>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: '600px' }}>
        ...
        <tbody>
          {Object.entries(substanceData).map(([year, data]) => (
            <tr key={year}>
