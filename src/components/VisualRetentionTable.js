...
import { calculateRetentionByVisualImpairment } from '@/lib/filterUtils';

export default function VisualRetentionTable({ processedData, filters }) {
  ...
  const calculateVisualData = () => {
    ...
    const retentionData = calculateRetentionByVisualImpairment(processedData, rawData);
  ...
  const visualData = calculateVisualData();
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontWeight: 'bold', marginRight: '15px' }}>Visual:</span>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: '600px' }}>
        ...
        <tbody>
          {Object.entries(visualData).map(([year, data]) => (
            <tr key={year}>
